...
func TestRolAcc(t *testing.T) {
    cpu := new(Cpu)
    tests := []cpuTest{
        {0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, cpu},
        {0x01, 0x00, 0x00, 0x01, 0x03, 0x00, 0x00, 0x00, 0x00, cpu},
        {0x40, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x80, 0x00, cpu},
        {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, cpu},
    }
...
func TestRol(t *testing.T) {
    cpu := new(Cpu)
    test := memTest{0x20, 0x40, 0x80, 0x80, cpu}
    test.setup()
    cpu.Rol(test.loc)
    test.test(t)
}

func TestRorAcc(t *testing.T) {
    cpu := new(Cpu)
    tests := []cpuTest{
        {0x02, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, cpu},
        {0x02, 0x00, 0x00, 0x01, 0x81, 0x00, 0x00, 0x80, 0x00, cpu},
        {0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x01, 0x00, cpu},
        {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, cpu},
    }
...
func TestRor(t *testing.T) {
    cpu := new(Cpu)
    test := memTest{0x30, 0x01, 0x00, 0x03, cpu}
    test.setup()
    cpu.Ror(test.loc)
    test.test(t)
}
